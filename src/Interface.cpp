...
void Menu::selectButton(sf::Text *Btn) { Btn->setFillColor(sf::Color::Green); }
...
Menu::Menu()
    : bg_img(dummy_texture), StartButton(dummy_text), SurvivalMode(dummy_text),
      Mute_Audio(dummy_text), QuitButton(dummy_text) {
  GameStarted = false;
...
PauseMenu::PauseMenu()
: continue_btn(dummy_text), restart_btn(dummy_text), mute_btn(dummy_text),
  quit_btn(dummy_text),
...
int PauseMenu::menu_loop(sf::RenderWindow &w) {
  ...
  while (const std::optional event = w.pollEvent()) {
    ...
    bool pressed_ret = (event->is<sf::Event::KeyReleased>() &&
                        keyReleased->code == Key::Enter);
    switch (selected_opt) {
    case Continue:
      if (pressed_ret)
